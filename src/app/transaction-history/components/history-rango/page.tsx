...
import { ResponseRangoPath } from "@/shared/Models/Rango";
...
export default function HistoryRango(props: propsType) {
    let [quoteDetail, setQuoteDetail] = useState<ResponseRangoPath>();
    ...
    useEffect(() => {
        ...
        const quote: ResponseRangoPath = JSON.parse(props?.detailViewData?.quoteDetail ?? '{}') as ResponseRangoPath;
        setQuoteDetail(quote);

        (async () => {
            console.log(quoteDetail);
            let fromAmount = await utilityService.convertToNumber(props?.detailViewData?.amount?.toString(), quote?.route?.swapper?.from?.decimals);
            let toAmount = await utilityService.convertToNumber((quote?.route?.outputAmount.toString()), quote?.route?.swapper?.to?.decimals);
            setQuoteDetail(quote);
                                            ...
                                            <div className="row justify-content-center g-3 align-items-center">
                                                <div className="col-md-5">
                                                    <div className="bg-white rounded p-3 text-center h-100">
                                                        ...
                                                        <strong className="d-block">{fromAmount} {quoteDetail?.route?.swapper?.from?.symbol}</strong>
                                                ...
                                                <div className="col-md-5">
                                                    <div className="bg-white rounded p-3 text-center h-100">
                                                        ...
                                                        <strong className="d-block">{toAmount} {quoteDetail?.route?.swapper?.to?.symbol}</strong>
                                                ...
                                                <div className="inner-card p-3 bg-light border rounded h-100">
                                                    ...
                                                    <div className="small">
                                                        <div className="mb-2">
                                                            ...
                                                            <span className="float-end fw-semibold">{fromAmount} {quoteDetail?.route?.swapper?.from?.symbol}</span>
                                                        </div>
                                                        <div className="mb-2">
                                                            <span className="text-muted">Sender:</span>
                                                            {
                                                                utilityService.isNullOrEmpty(quoteDetail?.tx?.from) &&
                                                                <>
                                                            ...
                                                            }
                                                            {
                                                                !utilityService.isNullOrEmpty(quoteDetail?.tx?.from) &&
                                                                <>
                                                                    <span className="float-end font-monospace">{`${quoteDetail?.tx?.from.slice(0, 6)}.....${quoteDetail?.tx?.from.slice(-4)}`}</span>
                                                                </>
                                                            }
                                            ...
                                            <div className="col-md-6">
                                                <div className="inner-card p-3 bg-light border rounded h-100">
                                                    ...
                                                    <div className="small">
                                                        <div className="mb-2">
                                                            <span className="text-muted">Amount:</span>
                                                            <span className="float-end fw-semibold">{toAmount} {quoteDetail?.route?.swapper?.to?.symbol}</span>
                                                        </div>
                                                        <div className="mb-2">
                              ...
                              <span className="float-end">06/14/2025 4:33 PM</span>
                                        ...
                                        <div className="inner-card p-3 bg-light border rounded">
                                            ...
                                            <div className="row small">
                                                ...
                                                <div className="col-6 mb-2 text-end">
                                                    <span> $ {quoteDetail?.route?.feeUsd} </span>
                                                </div>
                                                <div className="col-6 mb-2">
